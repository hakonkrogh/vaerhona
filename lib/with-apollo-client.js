  ...
  return class Apollo extends React.Component {
    ...
    static async getInitialProps(ctx) {
      ...
      if (App.getInitialProps) {
        ...
        if (!process.browser && Component.graphSettings) {
          try {
            ctx.ctx.graphData = apollo.cache.readQuery({
              query: Component.graphSettings.query,
              ...Component.graphSettings.options(ctx)
            });
          } catch (error) {
            console.error(
              'Error while trying to read from apollo cache',
              error
            );
          }
        }
