...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddSnapshot'), wrapper.instance().toggleAddSnapshotSection);
...
test('calling toggleAddSnapshotSection dispatches toggleAddSnapshot', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddSnapshotSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddSnapshot()));
});
